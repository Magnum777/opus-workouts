...
Usage: python unifi_do_it_now.py <MFA_CODE>
"""
import requests
import json
import sys
...
mfa_code = sys.argv[1] if len(sys.argv) > 1 else sys.exit("Usage: python unifi_do_it_now.py <MFA_CODE>")
...
print(f"\nAuthenticating with code {mfa_code}...")
r = session.post(
...
if r.status_code != 200:
    print(f"Authentication failed: {r.text[:500]}")
    sys.exit(1)
