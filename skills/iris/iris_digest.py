import os, imaplib, email, re, sys, json
...
URGENT_KW = ["urgent","asap","deadline","immediately","action required","time sensitive","overdue","past due","invoice","payment due","legal","lawsuit","critical","emergency","final notice","expires","expiring","last chance"]
REPLY_KW = ["?","question","can you","could you","please","request","following up","follow-up","reminder","let me know","thoughts"]
NOISE_PATTERNS = [r"unsubscribe",r"newsletter",r"no-reply@",r"noreply@",r"marketing@",r"notifications?@",r"donotreply@",r"@.*\.(mailchim|sendgrid|constantcontact|klaviyo)"]
NOISE_SUBJECTS = ["sale","% off","deal","offer","promo","subscribe","newsletter","weekly digest","monthly update","announcement"]
...
result = {"account": GMAIL_ADDR, "total": 0, "actionable": 0, "noise": 0, "high_priority": 0, "replied": 0, "items": []}
...
for uid in recent_ids:
    try:
        ...
        emails.append({"subject":subject,"sender":sname,"sender_email":semail,"age_hours":age_hours,"urgency":urgency,"is_noise":noise,"replied":has_replied})
    except: continue
...
result["noise"] = len(noise)
result["high_priority"] = sum(1 for e in actionable if e["urgency"]>=70)
result["replied"] = sum(1 for e in actionable if e["replied"])

for e in actionable[:10]:
    age = f"{int(e['age_hours'])}h" if e['age_hours']<48 else f"{int(e['age_hours']//24)}d"
    result["items"].append({"urgency": e["urgency"], "sender": clean(e["sender"][:25]), "subject": clean(e["subject"][:60]), "age": age, "replied": e["replied"]})
