#!/usr/bin/env python3
"""IMAP Gmail spam sweep - properly batched."""
...
def sweep_one(email_addr):
    ...
    print(f"  Checking {len(uids)} messages...")

    # PROPERLY BATCHED fetch
    ...
    for item in fetched:
        ...
        if not raw_bytes:
            continue
        # Check for UID in the response
        uid_match = re.search(rb"UID\s+(\d+)", raw_bytes if isinstance(raw_bytes, bytes) else b"")
        try:
    ...
    if spam_ids:
        # Use STORE + COPY per ID since IMAP batch copy is non-standard
        print(f"  Trashing {len(spam_ids)}...")
        for uid in spam_ids:
            if not uid:
                continue
            if time.time() - START_TIME > MAX_RUNTIME:
                print("  TIMEOUT during trash phase")
                break
            try:
                mail.uid("COPY", uid, "[Gmail]/Trash")
                mail.uid("STORE", uid, "+FLAGS", "\\\\Deleted")
            except Exception as e:
                print(f"    ERR on UID {uid}: {e}")
        mail.expunge()
    else:
        print("  Clean.")
...
if __name__ == "__main__":
    ...
    accounts = [target] if target and target in ACCOUNTS else list(ACCOUNTS.keys())
    ...
    if total_t > 0:
        print(f"\\n{'='*50}")
        print(f"Done. Checked ~{total_c}, trashed {total_t}. ({elapsed:.1f}s)")
        print(f"{'='*50}")
    sys.exit(0 if total_t == 0 else 1)
